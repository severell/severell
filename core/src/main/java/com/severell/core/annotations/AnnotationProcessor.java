...
public class AnnotationProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        try {
            ...
            HashMap<String, ArrayList<Element>> fileMap = sortIntoFiles(roundEnv.getElementsAnnotatedWith(Route.class));
            ...
            for(String file : fileMap.keySet()) {
                ...
                for(Element element : fileMap.get(file)){
                    routeIndex = processRoute(builder, routeIndex, element);
                    originatingElement = element.getEnclosingElement();
                }

                builder.addStatement("return list");
                writeRouteFile(file, builder, originatingElement);
            }
    ...
    private void writeRouteFile(String file, MethodSpec.Builder builder, Element element) throws IOException {
        String className = file.split("\\.")[file.split("\\.").length - 1];
        TypeSpec helloWorld = TypeSpec.classBuilder(className)
                .addJavadoc("GENERATED CLASS: This class is generated by the severell-maven-plugin. DO NOT EDIT")
                .addModifiers(Modifier.PUBLIC)
                .addAnnotation(RouteFile.class)
                .addMethod(builder.build())
                .build();


        JavaFile javaFile = JavaFile.builder("com.severell.core.generatedRoutes", helloWorld)
                .build();

        JavaFileObject fobj = filer.createSourceFile("com.severell.core.generatedRoutes." + className, element);

        try(Writer writer = fobj.openWriter()) {
            javaFile.writeTo(writer);
        }
    }

    private HashMap<String, ArrayList<Element>> sortIntoFiles(Set<? extends Element> annotations) {
        HashMap<String, ArrayList<Element>> map = new HashMap<>();
        for(Element element : annotations){
            String fileName = element.getEnclosingElement().toString();
            if(map.containsKey(fileName)) {
                map.get(fileName).add(element);
            } else {
                map.put(fileName, new ArrayList<Element>(Arrays.asList(element)));
            }
        }
        return map;
    }
